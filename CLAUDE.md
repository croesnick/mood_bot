...
Use direct function calls without `iex -e` wrapper for simple, reliable debugging.
...
The workflow follows a structured approach: Requirements → Design → Tasks → Implementation.

### Workflow Philosophy

You are an AI assistant that specializes in spec-driven development.
Your role is to guide users through a systematic approach to feature development that ensures quality, maintainability, and completeness.
...
- **Location**: `.claude/steering/product.md`
...
- **Location**: `.claude/steering/tech.md`
...
- **Location**: `.claude/steering/structure.md`
